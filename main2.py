...
def main(video_source="test1.mp4", save_video=False, show_video=True):
    ...
    if save_video:
        ...
        out_video_path = "output_main2.mp4"
    ...
    print("Using Tracker: ByteTrack (native YOLO)")
    ...
    while cap.isOpened():
        ...
        # 3. Jalankan YOLOv11 tracking dengan ByteTrack
        # classes=[0] untuk hanya mendeteksi "person"
        # persist=True agar ID tracking dipertahankan antar frame
        # tracker="bytetrack.yaml" secara eksplisit menggunakan ByteTrack (standar)
        results = model.track(frame, classes=[0], persist=True, tracker="bytetrack.yaml", conf=0.3, iou=0.5, verbose=False)
...
if __name__ == "__main__":
    ...
    # CCTV URLs
    cctv_sentral = f"rtsp://{user}:{password}@{host_sentral}/Streaming/Channels/401"
    cctv_lubeg = f"rtsp://{user}:{password}@{host_lubeg}/Streaming/Channels/101"

    sources = [
        cctv_sentral, # Index 0: CCTV Sentral
        cctv_lubeg,   # Index 1: CCTV Lubeg
        "test1.mp4",  # Index 2: File Video Lokal test1
        0             # Index 3: Webcam
    ]
    
    # Pilih source dengan merubah index di bawah
    source_index = 0
    video_input = sources[source_index]
    
    print(f"Starting ByteTrack People Counter with source: {video_input}")
